...
Dataset: Loan Default Prediction Dataset (255,347 rows, 18 columns) — sourced from Kaggle
...
Age — younger applicants are more likely to default
Interest Rate — higher rates increase default risk
Employment Type (Unemployed) — strongly increases default risk
Months Employed — longer employment tenure reduces risk
...
Methodology
Exploratory Data Analysis — checked for missing values (none found), reviewed value ranges, confirmed class imbalance
Preprocessing — dropped non-predictive ID column, binary-encoded Yes/No columns, applied One-Hot Encoding (with drop_first=True) to multi-category columns
Train/Test Split — 80/20 split with stratify to preserve class balance in both sets
Feature Scaling — StandardScaler fit on training data only (to avoid data leakage), applied to both train and test sets
Model Training & Comparison — trained and evaluated 5 models, prioritizing recall on the minority (default) class
Model Persistence — saved the final model, scaler, and feature column order with joblib
API Development — built a Flask REST API that replicates the exact preprocessing pipeline on new input before prediction
Containerization — packaged the API into a Docker image for consistent, portable local deployment
Notes
